...
#
# Usage: ./installGpmApps.sh [<sub-folder name>]
# Arguments [Optional]:
...
#		If the provided sub-folder does not exist, it will prompt the option to check in parent folder
...
if [ $( checkAdbDevice $deviceSerial ) == "true" ]; then
	...
	#	echo -e " ${txtRed}You are not allowed to install apps in @Home or ClockWork device${txtRst}\n"
		...
		installMachineFiles $deviceSerial ${myGPMDir} ${subFolder} "$searchString"
	#fi
else
